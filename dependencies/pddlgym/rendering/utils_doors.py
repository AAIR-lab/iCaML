    ...
    for r in range(height):
        for c in range(width):
            ...
            for i,im in enumerate(token_images):   
                if (i == 1 and layout[r,c,1]==1) or (i == 2 and layout[r,c,2]==1):
                    bndry = 1
                draw_token(im, r, c, ax, height, width,boundary = bndry)
                bndry = 0
...
def draw_token(token_image, r, c, ax, height, width, token_scale=1.0, fig_scale=1.0,boundary=False):
    if boundary:
        powe = 0.5
    else:
        powe = 0.45
    oi = OffsetImage(token_image, zoom = fig_scale * (token_scale / max(height, width,10)**powe))
