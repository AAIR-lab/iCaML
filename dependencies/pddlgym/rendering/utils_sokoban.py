...
def render_from_layout(layout, get_token_images,stone_dict):
    ...
    #     temp_stone_dict[str(v)] = int(k.replace('stone-',''))
    # stone_dict=temp_stone_dict
    
    for r in range(height):
        for c in range(width):
            im = get_token_images(layout[r, c])[0]
...
def draw_token(token_image, r, c, ax, height, width, token_scale=1.0, fig_scale=1.0):       
    oi = OffsetImage(token_image, zoom = fig_scale * (token_scale / max(height, width,10)**0.45))
